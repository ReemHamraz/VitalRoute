...
import {
  ...
  X, Info, Activity, Package, Radio
} from "lucide-react";

// ─────────────────────────────────────────────
// DESIGN: Cold War Teletype Command Terminal
// Fonts : Bebas Neue (display) · Courier Prime (data)
// Palette: warm charcoal · phosphor vermilion · verdigris
// Layout : hard CSS grid, 1px rule borders, NO glassmorphism
// Center : live radar sweep with incident blips
// ─────────────────────────────────────────────
  ...
  return (
    <div style={{ position: "relative", width: D, height: D, flexShrink: 0 }}>
      ...
      <div style={{ position: "absolute", inset: 0, borderRadius: "50%", overflow: "hidden", border: `1px solid ${C.ruleHard}` }}>
        ...
        {/* Sweep — conic gradient trail */}
        <div style={{
          position: "absolute", inset: 0, borderRadius: "50%",
          background: `conic-gradient(from 0deg at 50% 50%,
            transparent        0deg,
            rgba(224,56,32,0.0)  0deg,
            rgba(224,56,32,0.18) 52deg,
            rgba(224,56,32,0.05) 80deg,
            transparent        96deg)`,
      ...
      {/* Incident blips — positioned outside clipped div so labels can overflow */}
      {entries.map(e => {
      ...
      {/* Corner coords */}
      <span style={{ position: "absolute", bottom: 6, right: 10, fontFamily: mono, fontSize: 7, color: C.dim, letterSpacing: "0.1em" }}>34°03′N  84°21′W</span>
    </div>
  );
}
...
function DetailStrip({ entry }) {
  const u = URGENCY[entry.urgency] || URGENCY.LOW;
  return (
    <div style={{
      width: "100%", borderTop: `1px solid ${u.bd}`,
      background: u.dim, display: "flex", gap: 0,
      animation: "vr-reveal 0.3s ease both",
    }}>
      {/* Identity block */}
      <div style={{ padding: "12px 16px", borderRight: `1px solid ${C.rule}`, minWidth: 190, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 5 }}>
          <Ping color={u.color} sz={4} />
          <span style={{ fontFamily: mono, fontSize: 9, color: u.color, letterSpacing: "0.16em" }}>{u.label}</span>
        </div>
        <div style={{ fontFamily: bebas, fontSize: 22, color: C.text, letterSpacing: "0.06em", lineHeight: 1.05, marginBottom: 4 }}>{entry.title}</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {entry.items.map((item, i) => (
            <div key={i} style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
              <span style={{ fontFamily: bebas, fontSize: 16, color: u.color, letterSpacing: "0.05em", lineHeight: 1 }}>{item.q}</span>
              <span style={{ fontFamily: mono, fontSize: 9, color: C.mid, letterSpacing: "0.04em" }}>{item.u.toUpperCase()} {item.n}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
          {entry.flags.map((f, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 5 }}>
              <span style={{ width: 3, height: 3, borderRadius: "50%", background: u.color, flexShrink: 0 }} />
              <span style={{ fontFamily: mono, fontSize: 9, color: C.mid, letterSpacing: "0.08em" }}>{f.toUpperCase()}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "center", minWidth: 120, flexShrink: 0 }}>
        <button style={{
          ...
          background: u.dim, border: `1px solid ${u.bd}`,
          padding: "8px 14px", cursor: "pointer", display: "flex", alignItems: "center", gap: 6,
          transition: "background 0.2s",
        }}
          onMouseEnter={e => e.currentTarget.style.background = u.bd.replace("0.36","0.22").replace("0.38","0.22").replace("0.36","0.22")}
...
export default function VitalRoute() {
  ...
  async function extract() {
    ...
    const prompt = `Emergency logistics AI. Return ONLY valid JSON (no other text):
{"title":"<3-5 word incident name>","urgency":"<CRITICAL|HIGH|MODERATE|LOW>","loc":"<location or Location unspecified>","items":[{"n":"<item>","q":<number>,"u":"<units|vials|kits|bags|doses|packs>"}],"flags":["<tag>"]}
Request: "${input.trim()}"`;
    try {
      ...
      const entry = { id: Date.now(), ts, coords: { x: 32 + Math.random() * 36, y: 28 + Math.random() * 40 }, ...p };
      setNewId(entry.id); setActiveId(entry.id);
      setFeed(prev => [entry, ...prev]);
      setInput("");
      setTimeout(() => setNewId(null), 2000);
    } catch { setError("Parse failed — check API key / network."); }
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 0, flex: 1, paddingLeft: 1 }}>
            {[["NETWORK","LIVE",C.teal],["FEEDS","47",C.mid],["UNITS ACTIVE","12",C.mid],["SIGNAL","98%",C.teal]].map(([l,v,vc]) => (
        ...
        <div style={{
          ...
          {/* Detail strip — pinned to bottom of center */}
          {active && <DetailStrip key={activeId} entry={active} />}
        ...
        <div style={{
          ...
          {/* Scrollable feed */}
          <div ref={feedRef} className="vr-feed" style={{ flex: 1, overflowY: "auto" }}>
            {feed.map(e => (
